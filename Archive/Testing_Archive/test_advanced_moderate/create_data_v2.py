...
def first_gen(ethnicity):
	...
	if ethnicity is 'White':
		...
		out = thresholds(idd, outs, [0.9, 0.1])
	elif ethnicity is 'Asian':
		out = thresholds(idd, outs, [0.1, 0.9])
	return out

def languages(ethnicity):
	ethn_map = {'White':random.normalvariate(0.5, 0.25), 'Black':random.normalvariate(0.5, 0.25), 'Hispanic':random.normalvariate(1, 0.25), 'Asian':random.normalvariate(1, 0.25)}
...
def income_bracket(ethnicity):
	ethn_map={'White':random.normalvariate(0.5, 0.1), 'Black':random.normalvariate(0.4, 0.05), 'Hispanic':random.normalvariate(0.4, 0.05), 'Asian':random.normalvariate(0.4, 0.05)}
	idd = ethn_map[ethnicity]
	out = idd*120000
	if out < 10000:
		out = 10000
...
def undergrad(generation, ethn, sports, inc):
	generation_map = {'Yes':random.normalvariate(0.2, 0.03), 'No':random.normalvariate(0.8, 0.05)}
	ethn_map = {'White':random.normalvariate(0.5, 0.05), 'Black':random.normalvariate(0.4, 0.05), 'Hispanic':random.normalvariate(0.3, 0.03), 'Asian':random.normalvariate(0.8, 0.05)}
	sports_map = {'No':random.normalvariate(0.2, 0.03), 'Yes':random.normalvariate(0.5, 0.05)}
	if inc < 20000:
		inc_score = random.normalvariate(0.2, 0.03)
	elif inc < 70000:
		inc_score = random.normalvariate(0.5, 0.05)
	else:
		inc_score = random.normalvariate(0.8, 0.05)
...
def extra_degrees(generation, languages, inc):
	generation_map = {'Yes':random.normalvariate(0.3, 0.02), 'No':random.normalvariate(0.7, 0.03)}
	if inc < 20000:
		inc_score = random.normalvariate(0.2, 0.01)
	elif inc < 60000:
		inc_score = random.normalvariate(0.6, 0.05)
	else:
		inc_score = random.normalvariate(0.9, 0.03)
...
def law_school(undergrad, inc):
	if inc < 20000:
		inc_score = random.normalvariate(0.2, 0.02)
	elif inc < 60000:
		inc_score = random.normalvariate(0.5, 0.05)
	else:
		inc_score = random.normalvariate(0.8, 0.03)
	if undergrad < 20:
		U_score = random.normalvariate(0.8, 0.03)
	elif undergrad < 50:
		U_score = random.normalvariate(0.5, 0.05)
	else:
		U_score = random.normalvariate(0.2, 0.02)
...
def success(undergrad, lawschool, extradegrees, sports):
	if undergrad < 20:
		...
		U_score = random.normalvariate(0.2, 0.02)
	if lawschool < 20:
		L_score = random.normalvariate(0.8, 0.03)
	elif lawschool < 50:
		L_score = random.normalvariate(0.5, 0.05)
	else:
		L_score = random.normalvariate(0.2, 0.02)
	degree_score = extradegrees/5
	sports_map = {'No':random.normalvariate(0.2, 0.02), 'Yes':random.normalvariate(0.7, 0.04)}
	sports_score = sports_map[sports]
